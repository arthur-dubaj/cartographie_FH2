...
#On ajoute les pôles
nodes.append(poles[0])
nodes.append(poles[1])
nodes.append(poles[2])



#On relie les domaines principaux à leurs pôles
dico_placement = {}
nbr_dom_p_pole = {poles[0] : 0, poles[1] : 0, poles[2] : 0}
...
dico_coord_poles = {poles[0] : (1000,1000), poles[1] : (1000, 1000), poles[2] : (1000,1000)}
...
dicolor_poles = {
    poles[0] : '#03a3ff',     # Bleu FH principal
    poles[1] : '#88c000',     # Vert FH
    poles[2] : '#ff9e00'      # Orange FH
}
...
temp_dico = {poles[0] : 0, poles[1] : 0, poles[2] : 0}
...
def position2(nom, domaineFiltres, dico_angles):
    ...
    if nbr_domaine == 0:
        ...
        if nom in domaineFiltres or nom == 'Généraliste ':
            ...
            x = np.cos(np.radians(angle)) * 700
            y = np.sin(np.radians(angle)) * 700
            
        else:
            interaction = df.loc[df['Nom'] == nom, 'Interaction'].iloc[0]
            domaine_assoc = df.loc[df['Nom'] == nom, 'Domaine'].iloc[0]
            x0 = np.cos(np.radians(dico_pos_domaines[domaine_assoc])) * 700
            y0 = np.sin(np.radians(dico_pos_domaines[domaine_assoc])) * 700
...
for dp in domaine_principal_options:
    if dico_placement[dp] == poles[0]:
        domaine_principal_options_1.append(dp)

    if dico_placement[dp] == poles[1]:
        domaine_principal_options_2.append(dp) 

    if dico_placement[dp] == poles[2]:
